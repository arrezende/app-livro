...
interface SelectionMenuProps {
  ...
  onAction: (action: 'explain' | 'summarize' | 'translate' | 'identify') => void;
  onClose: () => void;
}

export const SelectionMenu: React.FC<SelectionMenuProps> = ({ visible, position, onAction, onClose }) => {
  ...
  const topPosition = showBelow ? position.y + position.height + 10 : position.y - 50;
  ...
  return (
    ...
    >
      ...
      <div className={`absolute left-1/2 -translate-x-1/2 w-3 h-3 bg-gray-900 rotate-45 ${showBelow ? '-top-1.5' : '-bottom-1.5'}`}></div>
    </div>
  );
};