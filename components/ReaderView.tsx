...
import { Book as BookType, Rendition as RenditionType, TocItem, Theme, AIResponseState } from '../types';
...
import { generateRecap, analyzeSelection, identifyCharacter, detectBookGenre } from '../services/geminiService';
...
export const ReaderView: React.FC<ReaderViewProps> = ({ 
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    try {
      ...
      if (savedThemeName) return themes.find(t => t.name === savedThemeName) || themes[0];
    } catch (e) { }
    return themes[0];
  });
  ...
  // Audio State
  const [audioState, setAudioState] = useState<{ loading: boolean; url: string | null; error: string | null }>({
  ...
  // Save preferences
  useEffect(() => { localStorage.setItem('lumina_theme', theme.name); }, [theme]);
  useEffect(() => { localStorage.setItem('lumina_font_size', fontSize.toString()); }, [fontSize]);
  ...
  useEffect(() => {
    ...
    if (viewerRef.current) {
      ...
      newBook.loaded.metadata.then(meta => {
        ...
        displayPromise.then(() => {
          if (initialCfi) showToast("Continuando de onde você parou", 'success');
          // Grace period
          setTimeout(() => { if (mounted) isLocationRestored.current = true; }, 1500);
        });
      }).then(() => {
      ...
      if (newBook.rendition && newBook.rendition.hooks) {
        ...
        newBook.rendition.hooks.content.register((contents) => {
            ...
            contents.document.addEventListener('contextmenu', (e: Event) => e.preventDefault());
      ...
      newRendition.on('selected', (cfiRange: string, contents: any) => {
        ...
        setTimeout(() => {
            setSelection({ visible: true, x, y, height: rect.height, text, cfiRange });
        }, 250);
      });
      ...
      newRendition.on('relocated', (location: any) => {
        ...
        saveTimeout.current = setTimeout(() => {
            if (location.start?.cfi) {
                // @ts-ignore
                const percent = book?.locations?.length() > 0 ? book.locations.percentageFromCfi(location.start.cfi) : 0;
  ...
  // ACTIONS
  const handleSelectionAction = async (action: 'explain' | 'summarize' | 'recap' | 'identify') => {
    ...
    if (action === 'identify') {
        ...
        try {
            const result = await identifyCharacter(name, metadata?.title || '', metadata?.creator || '');
            setCharacterAiState({ loading: false, content: result, error: null });
        } catch {
            setCharacterAiState({ loading: false, content: null, error: "Erro ao identificar." });
        }
        return;
    }
  ...
  const handleSmartRecap = async () => {
    ...
    try {
        ...
        setAiState({ loading: false, content: null, error: "Erro ao gerar resumo." });
    }
  };
  ...
  const handleGenerateAudio = async () => {
      ...
      try {
          ...
          if (!genre) {
             ...
             genre = await detectBookGenre(metadata?.title || '', metadata?.creator || '', snippet);
             setBookGenre(genre);
          }
          ...
          setAudioState({ loading: false, url: null, error: e.message || "Erro ao gerar áudio" });
      }
  };

  return (
    <div className="h-[100dvh] w-screen overflow-hidden flex flex-col transition-colors duration-500" style={{ backgroundColor: theme.bg }}>
      <Sidebar 
        toc={toc} 
      ...
      {/* Header */}
      <div className={`absolute top-0 left-0 p-4 z-40 transition-transform duration-300 ${showUI ? 'translate-y-0' : '-translate-y-full'}`}>
         <button 
            onClick={onBack}
      ...
      <Controls 
        ...
        onThemeChange={setTheme} onFontSizeChange={setFontSize}
        onAIAnalyze={handleSmartRecap}
        currentTheme={theme} fontSize={fontSize}
        currentChapterLabel={currentChapterLabel} showUI={showUI}
      />
