...
export const explainConcept = async (concept: string, context: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `No contexto deste trecho de livro: "${context.substring(0, 300)}...", explique brevemente o termo ou conceito: "${concept}". Responda em Português.`,
    });

    return response.text || "Não foi possível explicar o conceito.";
  } catch (error) {
    console.error("Gemini API Error:", error);
    throw new Error("Falha ao conectar com a IA.");
  }
...
Exemplo de tom seguro: "Severo Snape é o mestre de Poções em Hogwarts. Ele tem cabelos oleosos, nariz adunco e uma personalidade fria e sarcástica. Desde o início, demonstra uma forte antipatia por Harry Potter."
    ...
    const response = await ai.models.generateContent({
      ...
      contents: prompt,
      config: {
        tools: [{googleSearch: {}}], // Ativa busca para livros desconhecidos
      }
    });
